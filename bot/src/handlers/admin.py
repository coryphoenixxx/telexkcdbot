...
async def cb_users_info(call: CallbackQuery) -> None:
    ...
    users_info = await api.get_admin_users_info()
...
async def cb_toggle_spec_status(call: CallbackQuery) -> None:
    ...
    await api.toggle_spec_status(last_comic_id)
...
async def user_support_msg(msg: Message, state: FSMContext) -> None:
    ...
    is_banned = await api.get_ban_status(user_id)
    if not is_banned:
    ...
    await state.finish()
    await msg.answer(
        text=_("❗ <b>The message has been successfully sent to the admin.</b>"),
...
async def cb_answer(call: CallbackQuery, state: FSMContext, callback_data: dict) -> None:
    ...
    await bot.send_message(
        ADMIN_ID,
        text=f"❗ <b>Enter the answer to the user {user_id} (or /cancel):</b>",
    )
...
def register_admin_handlers(dp: Dispatcher) -> None:
    ...
    dp.register_message_handler(
        ...
        commands="cancel",
        state=[
            States.typing_admin_broadcast_msg,
            States.typing_admin_support_answer,
        ],
    )
    ...
    dp.register_message_handler(admin_support_msg, state=States.typing_admin_support_answer)
