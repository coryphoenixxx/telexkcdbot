...
Revision ID: 6fa68e25fa98
Revises:
Create Date: 2024-01-12 00:35:20.695336
...
def upgrade() -> None:
    ...
    op.create_index('uq_issue_number_if_not_none', 'comics', ['issue_number'], unique=True, postgresql_where=sa.text('issue_number IS NOT NULL'))
    ...
    op.create_index('uq_translation_title_if_not_draft', 'translations', ['language', 'title'], unique=True, postgresql_where=sa.text('NOT is_draft'))
...
def downgrade() -> None:
    ...
    op.drop_index('uq_translation_title_if_not_draft', table_name='translations', postgresql_where=sa.text('NOT is_draft'))
    op.drop_table('translations')
    op.drop_table('comic_tag_association')
    op.drop_table('tags')
    op.drop_index('uq_issue_number_if_not_none', table_name='comics', postgresql_where=sa.text('issue_number IS NOT NULL'))
