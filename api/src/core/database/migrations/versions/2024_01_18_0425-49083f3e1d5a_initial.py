...
Revision ID: 49083f3e1d5a
Revises:
Create Date: 2024-01-18 04:25:44.328772
...
def upgrade() -> None:
    ...
    op.create_index(
        ...
        ["issue_number"],
        unique=True,
        postgresql_where=sa.text("issue_number IS NOT NULL"),
    )
    op.create_index(
        "uq_title_if_extra",
        "comics",
        ["slug"],
        unique=True,
        postgresql_where=sa.text("issue_number IS NULL"),
    )
    op.create_table(
        "tags",
...
def downgrade() -> None:
    ...
    op.drop_table("tags")
    op.drop_index(
        "uq_title_if_extra", table_name="comics", postgresql_where=sa.text("issue_number IS NULL"),
    )
    op.drop_index(
        "uq_issue_number_if_not_extra",
        table_name="comics",
